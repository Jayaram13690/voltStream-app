"""
S3 Device Backend Client - Self-contained S3 access for AgentCore

This module provides direct S3 access for device state management,
specifically designed for AgentCore deployment. It contains all S3
configuration and logic within the agent package for easy deployment.
"""
...
DEVICE_BUCKET = "voltstream-storage"
DEVICE_KEY = "devices/devices.json"
...
class AgentS3DeviceClient:
    """Self-contained S3 client for AgentCore device management."""
    ...
    def _read_s3_json(self) -> List[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"[AGENT S3 READ] Bucket: {self.bucket}, Key: {self.key}")
            
            response = self.s3_client.get_object(Bucket=self.bucket, Key=self.key)
            content = response['Body'].read().decode('utf-8')
            devices = json.loads(content)
            
            logger.info(f"[AGENT S3 READ SUCCESS] Retrieved {len(devices)} devices")
            return devices
            
        except ClientError as e:
            error_code = e.response['Error']['Code']
            logger.error(f"[AGENT S3 READ ERROR] {error_code}: {str(e)}")
            ...
            logger.error(f"[AGENT S3 JSON PARSE ERROR] Invalid JSON in {self.bucket}/{self.key}: {str(e)}")
    ...
    def _write_s3_json(self, devices: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"[AGENT S3 WRITE] Uploading {len(devices)} devices to {self.bucket}/{self.key}")
            ...
            logger.info("[AGENT S3 WRITE SUCCESS] Upload successful")
            
        except ClientError as e:
            error_code = e.response['Error']['Code']
            logger.error(f"[AGENT S3 WRITE ERROR] {error_code}: {str(e)}")
    ...
    def get_device(self, device_id: int) -> Optional[Dict[str, Any]]:
        ...
        if device:
            logger.info(f"[AGENT GET DEVICE] Found device {device_id}: {device['name']}")
        else:
            logger.warning(f"[AGENT GET DEVICE] Device {device_id} not found")
    ...
    def update_device(self, device_id: int, status: str) -> Dict[str, Any]:
        ...
        if device is None:
            logger.error(f"[AGENT UPDATE ERROR] Device {device_id} not found")
            raise RuntimeError(f"Device with ID {device_id} not found")
        
        old_status = device['status']
        logger.info(f"[AGENT UPDATE] Device: {device['name']}, Old Status: {old_status}, New Status: {status}")
        ...
        logger.info(f"[AGENT UPDATE SUCCESS] Device {device_id} updated to {status}")
        return device
...
# Initialize the S3 client for AgentCore
agent_s3_client = AgentS3DeviceClient()


def get_devices() -> List[Dict[str, Any]]:
    """Get all devices directly from S3.
    ...
    Raises:
        RuntimeError: If S3 operation fails
    """
    devices = agent_s3_client.get_devices()
...
def update_device(device_id: int, status: str) -> Dict[str, Any]:
    """Update device status directly in S3.
    ...
    Raises:
        RuntimeError: If S3 operation fails or device not found
    """
    device = agent_s3_client.update_device(device_id, status)
...
def get_default_power() -> Dict[str, float]:
